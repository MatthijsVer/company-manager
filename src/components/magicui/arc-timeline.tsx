...
interface ArcTimelineProps extends ComponentPropsWithoutRef<"div"> {
  ...
  autoPlay?: {
    ...
    enabled?: boolean;
    /**
     * The interval between steps in milliseconds, default is 3000
     */
    interval?: number;
    /**
     * Whether to pause on hover, default is true
     */
...
export function ArcTimeline(props: ArcTimelineProps) {
  ...
  const {
    enabled: autoPlayEnabled = true,
    interval: autoPlayInterval = 2000,
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCurrentStepIndex((prevIndex) => {
        const nextIndex = (prevIndex + 1) % totalSteps;
        const nextRotation =
          -1 *
            nextIndex *
            angleBetweenMinorSteps *
            (lineCountFillBetweenSteps + 1) -
          angleBetweenMinorSteps * boundaryPlaceholderLinesCount;
        setCircleContainerRotateDeg(nextRotation);
        return nextIndex;
      });
    }, autoPlayInterval);
  ...
  const handleStepClick = (angle: number, stepGlobalIndex: number) => {
    setCircleContainerRotateDeg(-1 * angle);
    setCurrentStepIndex(stepGlobalIndex);
                ...
                return (
                  <div key={`${lineIndex}-${stepIndex}`}>
                    ...
                    <div
                      ...
                      onClick={() => handleStepClick(angle, stepGlobalIndex)}
                    >
                      <div
                        className={cn(
